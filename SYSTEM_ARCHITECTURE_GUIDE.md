...
# 13. سیستم احراز هویت و امنیت (Authentication & Security)

**مسیرها:** `backend/auth/security.py`, `backend/auth/models.py`, `backend/routes/auth.py`

سیستم احراز هویت CAMEO بر اساس استانداردهای بین‌المللی (NIST 800-63B) و OWASP طراحی شده است.

## 13.1. مکانیزم‌های امنیتی
- **رمزنگاری پسورد:** تمام پسوردها با الگوریتم **Bcrypt** (کاست فاکتور 12) هش می‌شوند و هیچ پسورد Plaintext ای ذخیره نمی‌شود.
- **تأیید پیچیدگی پسورد (Password Policy):** استانداردهای NIST الزام می‌کند که پسورد حداقل ۸ کاراکتر داشته و ترکیبی از حروف بزرگ، کوچک، اعداد، و کاراکترهای خاص باشد.
- **جلوگیری از Brute-Force:** سیستم دارای یک ماژول قفل حساب (Account Lockout) است که در جدول `login_attempts` ردیابی می‌شود. بعد از **۵ تلاش ناموفق**، اکانت به مدت **۱۵ دقیقه** قفل می‌شود.
- **محافظت CSRF:** از طریق کتابخانه `flask-wtf`، الگوی Double-Submit Cookie اجرا شده است (تولید توکن یکبارمصرف در رندر فرم و اعتبارسنجی در POST).
- **مدیریت نشست (Session Management):**
  - شناسه‌های نشست (Session Tokens) با آنتروپی ۳۸۴-بیتی (`secrets.token_urlsafe(48)`) تولید می‌شوند.
  - نشست‌ها فقط در کوکی‌های امن با فلگ‌های `HttpOnly` و `SameSite=Lax` نگهداری می‌شوند تا از حملات XSS جلوگیری شود.
  - **پنجره لغزان (Sliding Window):** توکن‌ها دارای مهلت انقضای ۳۰ دقیقه‌ای هستند که با هر بار تعامل کاربر، این مهلت تمدید می‌شود.
  - **ردیابی سمت سرور:** هر نشست در جدول `sessions` دیتابیس `global_auth.db` ذخیره می‌شود، بنابراین ادمین‌ها می‌توانند نشست‌های فعال کاربران را فوراً منقضی (Invalidate) کنند.

---

# 14. کنترل دسترسی مبتنی بر نقش (RBAC)

**مسیر:** `backend/auth/decorators.py`

سیستم دسترسی‌پذیری دارای ۴ لایه سلسله‌مراتبی و دکوراتورهای امنیتی در سطح API (Fail-Closed) است.

## 14.1. سطوح دسترسی (Roles)
1. **Super Admin (`super_admin`):**
   - دارای دسترسی کامل به تنظیمات زیرساختی سیستم.
   - مدیریت مستأجرها (Tenants / Companies).
   - **نقطه کور (Blind Spot):** به دلایل امنیتی و حریم خصوصی، Super Admin به دیتابیس داده‌های خصوصی شرکت‌ها (inventory, audit logs) دسترسی **ندارد**.
2. **Company Admin (`company_admin`):**
   - مدیر اصلی هر شرکت.
   - می‌تواند لیست کاربران در صف انتظار (`PENDING`) شرکت خود را تأیید کند.
   - امکان تغییر نقش کاربران، و تعلیق (Suspend) آن‌ها.
   - دسترسی کامل به تمام داشبوردها و ETL.
3. **Operator (`operator`):**
   - می‌تواند فایل‌های جدید ناموجود را از طریق سیستم ETL آپلود کند.
   - می‌تواند داده‌های تایید نشده (Review Queue) را حل‌وفصل (Resolve) کند.
4. **Viewer (`viewer`):**
   - دسترسی **فقط-خواندنی (Read-Only)**.
   - دکوراتور `@viewer_readonly` به شکل خودکار تمامی ریکوئست‌های `POST`, `PUT`, `DELETE` را برای این نقش مسدود می‌کند (`HTTP 403 Forbidden`).

---

# 15. معماری Multi-Tenancy (چند مستأجری / پایگاه داده پویا)

**هماهنگ‌کننده:** Hook میانی (Middleware) `before_request` در `app.py`

برای اینکه سیستم CAMEO به یک محصول SaaS لارج‌اسکیل تبدیل شود، معماری ایزوله‌سازی داده‌های شرکت‌ها (Data Isolation) پیاده‌سازی شده است.

## 15.1. استراتژی ایزوله‌سازی (Per-Tenant Database)
- سیستم دیتابیس تک‌فایلی `user.db` کنار گذاشته شده است.
- **دیتابیس مرکزی (`global_auth.db`):** 
  - شامل جداول `companies`، `users`، `sessions`، و `login_attempts`.
  - وظیفه احراز هویت اولیه و مسیریابی را بر عهده دارد.
- **دیتابیس‌های مستأجر (`{company_id}_user.db`):**
  - به هر شرکت ثبت‌شده، یک دیتابیس مجزا اختصاص می‌یابد (مثلاً دیتابیس مخصوص پتروشیمی الف با ب کاملاً از نظر فیزیکی و فایلی جداست).
  - این کار جلوی نشت داده‌ها (Data Leakage) در اثر باگ‌های احتمالی SQL-Injection را در سطح دیتابیس می‌گیرد.

## 15.2. جریان مسیریابی مستأجر (Tenant Routing)
1. کاربر لاگین می‌کند و شناسه لاگین او در Session کوکی ثبت می‌شود.
2. با هر ریکوئست، سیستم در روال `before_request` کاربر را در پایگاه داده شناسایی کرده و `company_id` او را استخراج می‌کند.
3. مقدار متغیر جهانی فلَسک رفرش می‌شود: `g.tenant_db_path = "backend/data/{company_id}_user.db"`.
4. تمام توابع اتصالی (مثل `get_user_db_connection`) به جای مسیر استاتیک، دیتابیس کاربر را دقیقاً از روی `g.tenant_db_path` لود می‌کنند. 
5. دیتابیس مستأجر در اولین لاگین کاربر به‌صورت خودکار (Provisioning) متولد و مقداردهی (Migrate) می‌شود.

---

# 16. رابط کاربری پنل ادمین و تایید کاربران (UI/UX)

**قالب‌ها:** `login.html`, `register.html`, `admin_users.html`, `base.html`

برای تغییر از یک ابزار دسکتاپ به یک سرویس Enterprise ابری، ظاهر صفحات حیاتی (احراز هویت و مدیریت) کاملا دگرگون شده‌اند.

## 16.1. طراحی Industrial Glassmorphism
- تمامی پنل‌های لاگین، ثبت‌نام و مدیریت کاربران با تکنیک **گلس‌مورفیسم** (برگرفته از macOS/VisionOS) طراحی شده‌اند.
- پنل‌ها دارای تاری پس‌زمینه (`backdrop-blur-lg`)، حاشیه‌های شفاف بی‌نهایت (`border-white/10`) و سایه‌های عمیق نوری (`shadow-2xl`) هستند.
- استیت‌های UI (Idle, Loading, Success, Error, Lockout) با ترانزیشن‌های نرم توسط `Alpine.js` مدیریت می‌شوند.

## 16.2. جریان تایید کاربر (User Approval Workflow)
1. افراد ثبت نام می‌کنند (وضعیت `PENDING` می‌گیرند).
2. صفحه مدیریت کاربران (`/admin/users`) با اسکریپت آسنکرون، وضعیت کاربران `PENDING` را فقط برای Admin ها استخراج می‌کند. یک **Badge هشدار سیستم** در نوار کناری ظاهر می‌شود.
3. در قالب `admin_users.html`، ادمین شرکت نقش فرد را تعیین کرده و دکمه `Approve` را می‌زند تا حساب از انتظار خارج و فعال گردد.
4. از همان جدول کاربران امکان `Suspend` کردن اکانت فعال (جهت مسدود کردن آنی مجوز در حوادث امنیتی) لحاظ شده است.
